...
car_color = (143, 0, 0)
truck_color = (69, 0, 0)
rock_color = (180,130,70)
def get_car_mask(img):
    result = cv2.inRange(img, truck_color, car_color)
    # result2 = cv2.inRange(img,rock_color,rock_color)
    # result = cv2.bitwise_or(result,result2)
...
def start_turning(bird_view_img, direction):
    ...
    # global right_cors
    dir = (direction + 1) * 159
    turn_left_cors = get_line_cor((139, 159), (100, dir)).T
    turn_right_cors = get_line_cor((159, 160), (130, dir)).T    
...
kernel = kernel.astype(np.uint8)
def get_steer_no_mask(sign_status,img):
    global turning_frame
    global turn_dir
    global list_signs
    img = get_bird_view(img)
    img = img.astype(np.uint8)
    angle = (get_confident_vectors(img)) * 25
    ...
    if turning_frame > 0:
        #turning ...
        turning_frame -= 1
        angle_bias = (start_turning(img , turn_dir))
        print(angle_bias)
        if angle_bias < 0:
            angle_bias = 0
...
def get_sign(img):
    return -1
def get_steer(sign_status, mask):
    # global turn_thresh
    global turning_frame
    global turn_dir
    global list_signs
