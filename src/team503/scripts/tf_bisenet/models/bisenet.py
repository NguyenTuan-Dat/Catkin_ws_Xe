...
from tf_bisenet.builders import frontend_builder
import numpy as np
...
class BiseNet(object):
    ...
    def build_inputs(self, input_img):
        ...
        new_shape = tf.squeeze(tf.stack([[320], [320]]), axis=[1])
        self.images = tf.image.resize_images(input_img, new_shape)
        self.images.set_shape((320, 320, 3))
        self.images = tf.to_float(self.images)
        self.images = tf.image.rgb_to_hsv(self.images)
        self.images = tf.expand_dims(self.images, axis=0)

        #
        #     self.images =self.images_feed
    ...
    def build_loss(self):
        ...
        # tf.summary.image('Laplacian', self.images, family=self.mode, max_outputs=1)
        tf.summary.image('RGB', self.images, family=self.mode, max_outputs=1)
    ...
    def build(self, input, reuse=False):
        """Creates all ops for training and evaluation"""
        with tf.name_scope(self.mode):
            self.build_inputs(input)
